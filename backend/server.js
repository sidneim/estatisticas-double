...
app.get('/api/stats/blaze', async (req, res) => {
  try {
    const { rows } = await pool.query('SELECT * FROM resultdoubleblaze ORDER BY created_at DESC LIMIT 100');
    res.json(rows);
  } catch (err) {
...
app.get('/api/stats/jonbet', async (req, res) => {
  try {
    const { rows } = await pool.query('SELECT * FROM resultdoublejon ORDER BY created_at DESC LIMIT 100');
    res.json(rows);
  } catch (err) {
